...
const ovrly = {
	open: function (link) {
		...
		overlay.setAttribute(
			...
			'position:fixed;z-index:' +
				...
				';top:' +
				yOffset +
				'px;left:0;width:100%;height:100%;background-color:rgba(0,0,0,.5);text-align:center;box-sizing:border-box;padding:50px;',
		)
		let image = document.createElement('img')
		image.setAttribute('src', link.src)
		image.setAttribute(
			'style',
		...
		closeButton.setAttribute(
			...
			'position:absolute;z-index:' +
				(zIndex + 1) +
				';top:0;right:0;text-decoration:none;color:#fff;font-size:30px;padding:10px 15px 10px 10px;',
		)
		...
		document.addEventListener('keydown', ovrly.close)

		window.addEventListener('scroll', ovrly.close)

		document.getElementById('ovrly').addEventListener('click', ovrly.close)
	...
	close: function (event) {
		...
		if (fire == true) {
			...
			document.removeEventListener('keydown', ovrly.close)

			window.removeEventListener('scroll', ovrly.close)
		}
